...
[MCP](https://modelcontextprotocol.io) server exposing the Airtable SDK as
two agent tools — `airtable_list` and `airtable_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/airtable-mcp)
make build

# 2. Provide credentials via the environment
export AIRTABLE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user airtable \
  -- /absolute/path/to/airtable-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./airtable-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// airtable_list: first page of records
{ "entity": "base" }
{ "entity": "base", "query": { } }

// airtable_load: one record by id
{ "entity": "get_record", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/airtable-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user airtable \
     -- "$PWD"/dist/*/airtable-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `airtable_list` and `airtable_load` tools now appear
   in new sessions. Ask the agent to *"list base using airtable"*
   and it calls `airtable_list` with `{"entity":"base"}`.
...
```sh
export AIRTABLE_APIKEY=sk_live_xxx            # API key
export AIRTABLE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `airtable_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `airtable_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AIRTABLE_APIKEY` | API key sent with every request. |
| `AIRTABLE_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

base | create_record | get_record | list_record | table

### Smoke test via HTTP (raw JSON-RPC)

```sh
./airtable-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"airtable_load","arguments":{"entity":"get_record","query":{"id":1}}}}'
```
