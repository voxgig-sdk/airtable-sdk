...
type Args struct {
	Entity string         `json:"entity" jsonschema:"base | create_record | get_record | list_record | table"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AirtableSDK) {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "airtable_load",
		Description: "Load a single record from Airtable. " +
...
func runOp(client *sdk.AirtableSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AirtableSDK, name string) (sdk.AirtableEntity, error) {
	switch strings.ToLower(name) {
	case "base":
		return client.Base(nil), nil
	case "create_record":
		return client.CreateRecord(nil), nil
	case "get_record":
		return client.GetRecord(nil), nil
	case "list_record":
		return client.ListRecord(nil), nil
	case "table":
		return client.Table(nil), nil
