# Airtable Lua SDK Reference

Complete API reference for the Airtable Lua SDK.
...
```lua
local create_record = client:CreateRecord(nil)
```
...
```lua
local result, err = client:CreateRecord():create({
  base_id = --[[ string ]],
  table_id = --[[ string ]],
})
```
...
```lua
local get_record = client:GetRecord(nil)
```
...
```lua
local result, err = client:GetRecord():load({ base_id = "base_id", record_id = "record_id", table_id = "table_id" })
```
...
```lua
local list_record = client:ListRecord(nil)
```
...
```lua
local results, err = client:ListRecord():list()
```
...
```lua
local result, err = client:Table():create({
  base_id = --[[ string ]],
})
```
...
```lua
local results, err = client:Table():list()
```
