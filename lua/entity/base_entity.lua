...
function BaseEntity.new(client, entopts)
  entopts = entopts or {}
...
function BaseEntity:get_name()
  return self._name
end


function BaseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BaseEntity.new(self._client, opts)
end
...
function BaseEntity:mark_deleted()
  self._deleted = true
end


function BaseEntity:deleted()
  return true == self._deleted
end


function BaseEntity:data_set(args)
  if args ~= nil then
...
function BaseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BaseEntity:match_set(args)
  if args ~= nil then
...
function BaseEntity:stream(action, args, callopts)
...
---@param reqmatch BaseListMatch
---@param ctrl? table
---@return Base[]
---@return string? err
function BaseEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function BaseEntity:_run_op(ctx, post_done)
  local utility = self._utility
