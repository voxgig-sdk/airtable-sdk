...
local TableEntity = {}
TableEntity.__index = TableEntity


function TableEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TableEntity)
  self._name = "table"
  self._client = client
...
function TableEntity:get_name()
  return self._name
end


function TableEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TableEntity.new(self._client, opts)
end
...
function TableEntity:mark_deleted()
  self._deleted = true
end


function TableEntity:deleted()
  return true == self._deleted
end


function TableEntity:data_set(args)
  if args ~= nil then
...
function TableEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TableEntity:match_set(args)
  if args ~= nil then
...
function TableEntity:match_get()
...
function TableEntity:stream(action, args, callopts)
...
---@param reqmatch TableListMatch
---@param ctrl? table
---@return Table[]
---@return string? err
function TableEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TableCreateData
---@param ctrl? table
---@return Table
---@return string? err
function TableEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function TableEntity:_run_op(ctx, post_done)
  local utility = self._utility
