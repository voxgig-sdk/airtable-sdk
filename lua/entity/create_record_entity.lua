...
local CreateRecordEntity = {}
CreateRecordEntity.__index = CreateRecordEntity


function CreateRecordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CreateRecordEntity)
  self._name = "create_record"
  self._client = client
...
function CreateRecordEntity:get_name()
  return self._name
end


function CreateRecordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CreateRecordEntity.new(self._client, opts)
end
...
function CreateRecordEntity:mark_deleted()
  self._deleted = true
end


function CreateRecordEntity:deleted()
  return true == self._deleted
end


function CreateRecordEntity:data_set(args)
  if args ~= nil then
...
function CreateRecordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CreateRecordEntity:match_set(args)
  if args ~= nil then
...
function CreateRecordEntity:match_get()
...
function CreateRecordEntity:stream(action, args, callopts)
...
---@param reqdata CreateRecordCreateData
---@param ctrl? table
---@return CreateRecord
---@return string? err
function CreateRecordEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CreateRecordEntity:_run_op(ctx, post_done)
  local utility = self._utility
