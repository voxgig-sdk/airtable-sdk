...
local GetRecordEntity = {}
GetRecordEntity.__index = GetRecordEntity


function GetRecordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetRecordEntity)
  self._name = "get_record"
  self._client = client
...
function GetRecordEntity:get_name()
  return self._name
end


function GetRecordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetRecordEntity.new(self._client, opts)
end
...
function GetRecordEntity:mark_deleted()
  self._deleted = true
end


function GetRecordEntity:deleted()
  return true == self._deleted
end


function GetRecordEntity:data_set(args)
  if args ~= nil then
...
function GetRecordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetRecordEntity:match_set(args)
  if args ~= nil then
...
function GetRecordEntity:match_get()
...
function GetRecordEntity:stream(action, args, callopts)
...
---@param reqmatch GetRecordLoadMatch
---@param ctrl? table
---@return GetRecord
---@return string? err
function GetRecordEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetRecordEntity:_run_op(ctx, post_done)
  local utility = self._utility
