...
// Airtable SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class AirtableMakePoint
{
    public static function call(AirtableContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof AirtableError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = AirtableHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = AirtableHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
