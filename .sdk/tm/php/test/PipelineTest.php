...
{
    private static function utility(): AirtableUtility
    {
        return new AirtableUtility();
    }
    ...
    private static function ctx(array $over = []): AirtableContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AirtableContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AirtableOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AirtableError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AirtableOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AirtableMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AirtableOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AirtableMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AirtableOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AirtableMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirtableSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AirtableMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AirtableResponse([]),
            'result' => new AirtableResult([]),
        ]);
        [, $err] = AirtableMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AirtableSpec([]),
            'response' => null,
            'result' => new AirtableResult([]),
        ]);
        [, $err] = AirtableMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AirtableSpec(['step' => 's']),
            'response' => new AirtableResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AirtableResult([]),
        ]);
        [, $err] = AirtableMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AirtableSpec(['step' => 's']),
            'response' => new AirtableResponse(self::resp(200, ['v' => 1])),
            'result' => new AirtableResult([]),
        ]);
        [, $err] = AirtableMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AirtableSpec(['step' => 's']),
            'response' => new AirtableResponse(self::resp(200, ['v' => 2])),
            'result' => new AirtableResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AirtableMakeResponse::call($ctx);
    ...
    {
        $preset = new AirtableResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AirtableSpec([]),
            'response' => new AirtableResponse([]),
            'result' => new AirtableResult([]),
        ...
        [$got, $err] = AirtableMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AirtableResult([])]);
        [, $err] = AirtableMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AirtableSpec([]), 'result' => null]);
        [, $err] = AirtableMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirtableSpec(['step' => 's']),
            'result' => new AirtableResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AirtableOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirtableMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AirtableSpec(['step' => 's']),
            'result' => new AirtableResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AirtableOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AirtableMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AirtableResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AirtableSpec([]),
            'result' => new AirtableResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AirtableMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirtableMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AirtableError('boom', 'boom');
        $utility->fetcher = function (AirtableContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirtableSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AirtableMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AirtableContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AirtableMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AirtableResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AirtableContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AirtableSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AirtableMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AirtableResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AirtableSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AirtableMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AirtableMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AirtableMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AirtableResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AirtableDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AirtableResult(['ok' => false])]);
        $this->expectException(AirtableError::class);
        AirtableDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AirtableResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AirtableMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AirtableMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AirtableBaseFeature();
        $b = new AirtableBaseFeature();
        AirtableFeatureAdd::call($ctx, $a);
        AirtableFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AirtableBaseFeature
    {
        $f = new AirtableBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AirtableFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AirtableContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AirtableSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AirtablePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AirtableResponse(['headers' => null]),
            'result' => new AirtableResult([]),
        ]);
        AirtableResultHeaders::call($ctx);
