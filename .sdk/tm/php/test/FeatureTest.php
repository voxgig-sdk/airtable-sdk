...
class FtCtrl extends AirtableControl
{
...
class FtClient
{
    ...
    private AirtableUtility $utility;

    public function __construct(AirtableUtility $utility, array $options)
    {
    ...
    public function get_utility(): AirtableUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AirtableContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AirtableContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AirtableContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AirtableConfig::make_feature($name);
    ...
    private static function buildUrl(AirtableSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AirtableError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AirtableResponse($fetched);
        }
    ...
    private function fail(AirtableContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AirtableContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
