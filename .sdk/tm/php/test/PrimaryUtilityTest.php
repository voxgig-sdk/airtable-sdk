...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        AirtableSDK $client,
        AirtableUtility $utility
    ): AirtableContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new AirtableContext($ctxmap, null);
    ...
    private static function fixctx(AirtableContext $ctx, AirtableSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?AirtableError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AirtableError($code, $msg);
    }
    ...
    {
        $client = AirtableSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new AirtableBaseFeature();
    ...
    {
        $hook_client = AirtableSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends AirtableBaseFeature {
            public $hook_fn = null;
            public function TestHook(AirtableContext $ctx): void
            {
    ...
    {
        $init_client = AirtableSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends AirtableBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new AirtableSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new AirtableSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = AirtableSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = AirtableSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof AirtableError) ? $result->err->msg : (string)$result->err;
