...
const TEST_SEED = {"entity":{"base":{"test01":{"id":"test01"}},"create_record":{"test01":{"id":"test01"}},"get_record":{"test01":{"id":"test01"}},"list_record":{"test01":{"id":"test01"}},"table":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+AirtableSDK\b/.test(code) ||
    /\bAirtableSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AirtableSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AirtableSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
