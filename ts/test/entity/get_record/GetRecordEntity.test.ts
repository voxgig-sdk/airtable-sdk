...
describe('GetRecordEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'get_record.' + op, live)) return
    }
    ...
    let get_record_ref01_data = Object.values(setup.data.existing.get_record)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['AIRTABLE_TEST_GET_RECORD_ENTID']
