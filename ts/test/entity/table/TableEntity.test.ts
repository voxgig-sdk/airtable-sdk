...
describe('TableEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list']) {
      if (maybeSkipControl(t, 'entityOp', 'table.' + op, live)) return
    }
    ...
    // LIST
    const table_ref01_match: any = {}
    table_ref01_match['base_id'] = setup.idmap['base01']

    const table_ref01_list = (await table_ref01_ent.list(table_ref01_match)).map((e: any) => e.data())
