...
describe('BaseEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'base.' + op, live)) return
    }
    ...
    let base_ref01_data = Object.values(setup.data.existing.base)[0] as any

    // LIST
    const base_ref01_ent = client.Base()
    const base_ref01_match: any = {}

    const base_ref01_list = (await base_ref01_ent.list(base_ref01_match)).map((e: any) => e.data())
