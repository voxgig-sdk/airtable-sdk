...
The TypeScript SDK for the Airtable API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Base()` — each with a small set of operations (`list`, `load`, `create`)
...
`list()` resolves to an array of Base ENTITIES — every operation
...
```ts
const client = new AirtableSDK({ apikey: '...' })
...
| Method | Returns | Description |
| --- | --- | --- |
| `AirtableSDK.test(testopts?, sdkopts?)` | `AirtableSDK` | Create a test-mode client. |
...
| `client` | `client(): AirtableSDK` | Return the parent SDK client. |
...
Create an instance: `const base = client.Base()`
...
```ts
const tables = await client.Table().list({ base_id: "example" })
```
