...
boru-driven command-line client **and** interactive REPL for the Airtable
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/airtable-cli)
make build
...
./airtable-cli list base
./airtable-cli list create_record

# 5. Override the API base URL for a single call
AIRTABLE_BASE=https://api.example.com ./airtable-cli list base
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/airtable-cli
   ```
...
Run with no arguments to open a REPL (prompt `airtable>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `base`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/airtable-cli`. |
...
(`./airtable-cli <expr>`) and interactively (the REPL), and expressions compose the
