// Command Airtable-cli is a CLI + REPL for the Airtable SDK,
...
const prompt = "airtable"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "base create_record get_record list_record table"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AIRTABLE_APIKEY (api key), AIRTABLE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AIRTABLE_APIKEY carries the API key and
	// AIRTABLE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AIRTABLE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AIRTABLE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAirtableSDK(opts)
