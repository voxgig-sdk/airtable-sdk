...
class TableDirectTest extends TestCase
{
    public function test_direct_list_table(): void
    {
        $setup = table_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-table", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["base01"] as $_liveKey) {
...
function table_direct_setup($mockres)
{
