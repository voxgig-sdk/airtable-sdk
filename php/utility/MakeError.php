...
class AirtableMakeError
{
    public static function call(?AirtableContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AirtableContext([], null);
        }
        $op = $ctx->op ?? new AirtableOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AirtableResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AirtableError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AirtableError('', $msg, $ctx);
