...
// Airtable SDK CreateRecord entity
...
class CreateRecordEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CreateRecordEntity($this->_client, $opts);
    }

    /**
     * @param CreateRecord|array $args CreateRecord data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AirtableHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CreateRecord|array The current CreateRecord data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CreateRecord fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AirtableHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CreateRecord fields).
     */
    ...
    /**
     * Create a new CreateRecord.
     *
     * @param CreateRecordCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed CreateRecordCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CreateRecord|array The created CreateRecord as an assoc-array at the
     *   SDK boundary; throws AirtableError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = AirtableHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
