...
class TestTableEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Table(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Table(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AIRTABLE_TEST_TABLE_ENTID JSON to run live")
        client = setup["client"]
        ...
        table_ref01_data = helpers.to_map(runner.entity_data(table_ref01_ent.create(table_ref01_data, None)))
        assert table_ref01_data is not None
        assert table_ref01_data["id"] is not None
        ...
        table_ref01_list_result = table_ref01_ent.list(table_ref01_match, None)
        assert isinstance(table_ref01_list_result, list)
...
def _table_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("AIRTABLE_TEST_TABLE_ENTID"))
