...
class TestBaseDirect:

    def test_should_direct_list_base(self):
        setup = _base_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-base", "live" if setup["live"] else "unit")
        if _skip:
...
def _base_direct_setup(mockres):
