# Airtable JavaScript SDK Reference

Complete API reference for the Airtable JavaScript SDK.
...
```ts
new AirtableSDK(options?: object)
```
...
#### `AirtableSDK.test(testopts?, sdkopts?)`
...
**Returns:** `AirtableSDK` instance in test mode.
...
Create a new `Base` entity instance.
...
```ts
const results = await client.Base().list()
```
...
```ts
const results = await client.ListRecord().list()
```
...
```ts
const results = await client.Table().list()
```
